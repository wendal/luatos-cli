...
| CH340X 引脚 | 连接目标 | 说明 |
|-------------|----------|------|
| 6# (DTR) | MCU BOOT0 + 4.7kΩ 下拉到 GND | DTR LOW → BOOT0 HIGH（实测有效） |
| RTS# | MCU RESET（低有效） | RTS# LOW → RESET 拉低 |

> **实测说明**：根据实际电路，`DTR=LOW`（软件 false）时 BOOT0 被拉高进入 ROM BL；  
> 这是 `--dtr-boot` 的默认值。若你的硬件接法相反，可用 `--dtr-boot high` 覆盖。
...
|------|----------------|-------------|----------|------|
| 1 | `write_data_terminal_ready(false)` | DTR 输出 LOW | BOOT0 = HIGH | 预置引导模式（实测有效） |
...
| 步骤 | serialport API | CH340X 引脚 | MCU 引脚 | 说明 |
|------|----------------|-------------|----------|------|
| 1 | `write_data_terminal_ready(true)` | DTR 输出 HIGH | BOOT0 = LOW | 预置正常启动 |
| 2 | `write_request_to_send(true)` | RTS# 输出 LOW | RESET = LOW | 拉低复位 |
| 3 | sleep 100ms | — | RESET 保持低 | — |
...
```bash
# 全量刷机（CH340X 自动进入/退出 ROM BL，使用默认电平：DTR=LOW, RTS=HIGH）
...
# 调试：改用 DTR HIGH → BOOT0（若默认 LOW 不起效）
luatos-cli flash run --soc firmware.soc --port COM13 --auto-reset --dtr-boot high

# 调试：反转 RTS 极性（RTS LOW → 某些硬件的 RESET 有效）
luatos-cli flash run --soc firmware.soc --port COM13 --auto-reset --rts-reset low

# 调试：延长复位脉冲和 boot 等待时间
luatos-cli flash run --soc firmware.soc --port COM13 --auto-reset --reset-ms 200 --boot-wait-ms 800
```

| 参数 | 默认值 | 说明 |
|------|--------|------|
| `--auto-reset` | false | 启用自动复位（必须加此参数才会触发 DTR/RTS 操作） |
| `--dtr-boot` | `low` | 进入 boot 时 DTR 的电平；`low` → DTR 输出 LOW → BOOT0 = HIGH（实测有效） |
| `--rts-reset` | `high` | 触发复位时 RTS 的电平；`high` → CH340X RTS# 输出 LOW → RESET 有效 |
| `--reset-ms` | 100 | 复位脉冲宽度（毫秒） |
| `--boot-wait-ms` | 500 | 进入 boot 后等待 ROM BL 初始化的时长（毫秒） |
...
| 下载口与日志口 | **同一串口**（最新固件，如 COM13） |

> **串口说明**：最新版 LuatOS-SF32LB58 固件将下载口和日志口合并为同一个串口（如 COM13）。  
> 刷机和查看日志均使用该串口，无需切换端口。旧版固件可能使用两个不同串口（COM13 刷机，COM12 日志）。
