...
```bash
# 全量刷机（CH340X 自动进入/退出 ROM BL，使用默认电平）
...
# 调试：反转 DTR 极性（DTR LOW → BOOT0 拉高）
luatos-cli flash run --soc firmware.soc --port COM13 --auto-reset --dtr-boot low
...
| `--dtr-boot` | `high` | 进入 boot 时 DTR 的电平；`high` → DTR 输出 HIGH → BOOT0 = HIGH |
