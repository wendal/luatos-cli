...
//   Air1601 / CCM4211        — full via Rust OTA (LZMA compress + assemble), --script-only for script-only update
...
fn build_ec7xx_fota(new_soc: &str, old_soc: &str, chip: &str, toolkit_path: &Path, toolkit_dir: &Path, out_path: &Path) -> Result<()> {
    ...
    // Assemble OTA package using Rust implementation (replaces soc_tools make_ota_file)
    luatos_soc::ota::assemble_ota_package(
        0,       // magic
        0,       // main_version_decimal
        "0",     // main_md5_prefix
        0,       // std_version_decimal
        "0",     // std_md5_prefix
        0,       // app_version
        &dummy,  // common partition (dummy)
        &delta,  // SDK partition (delta.par)
        out_path,
    ).context("assemble OTA package")?;
...
fn build_ccm4211_fota(new_soc: &str, out_path: &Path) -> Result<()> {
    ...
    let magic = parse_hex_addr(magic_str)
        .ok_or_else(|| anyhow::anyhow!("invalid fota.magic_num: {magic_str}"))? as u32;

    let app_addr_str = info.download.app_addr.as_deref().unwrap_or("20000000");
    let app_addr = parse_hex_addr(app_addr_str)
        .ok_or_else(|| anyhow::anyhow!("invalid download.app_addr: {app_addr_str}"))? as u32;
    ...
    luatos_soc::ota::lzma_compress_file(rom_bin, &ap_zip, magic, app_addr, 0x40000, true)
        .context("compress ROM for OTA")?;

    let total_zip = tmp.path().join("total.zip");
    if script_bin.exists() {
        if let Some(saddr_str) = script_addr {
            let saddr = parse_hex_addr(saddr_str)
                .ok_or_else(|| anyhow::anyhow!("invalid download.script_addr: {saddr_str}"))? as u32;
            let s_zip = tmp.path().join("s.zip");
            luatos_soc::ota::lzma_compress_file(&script_bin, &s_zip, magic, saddr, 0x40000, true)
    ...
    luatos_soc::ota::assemble_ota_package(
        magic, 0xFFFFFFFF, "0", 0, "0", 0,
        &total_zip, &dummy, out_path,
    ).context("assemble OTA package")?;
...
fn build_ccm4211_script_only_fota(new_soc: &str, out_path: &Path) -> Result<()> {
    ...
    let script_addr = parse_hex_addr(script_addr)
        .ok_or_else(|| anyhow::anyhow!("invalid download.script_addr: {script_addr}"))? as u32;

    let script_bin = up.dir.join(&info.script.file);
    anyhow::ensure!(script_bin.exists(), "script file not found: {} — script-only FOTA requires a script partition", script_bin.display());
...
pub fn cmd_fota_build(
    ...
    match chip {
        ...
        // ── Air1601 / CCM4211 — full or script-only ─────────────────────────
        "air1601" | "ccm4211" => {
            if script_only {
                let out_path: PathBuf = output.map(PathBuf::from).unwrap_or_else(|| PathBuf::from(format!("{chip}_fota.sota")));
        ...
        other => bail!(
            ...
            Supported: EC7xx/EC618/Air8000 (differential), Air1601/CCM4211 (full or --script-only), Air6208/XT804 (full)."
        ),
    }
