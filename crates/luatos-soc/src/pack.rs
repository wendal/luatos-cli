...
pub fn pack_soc_zip(dir: &Path, out_path: &str) -> Result<()> {
    let file = fs::File::create(out_path).with_context(|| format!("Cannot create output: {out_path}"))?;
...
fn add_dir_to_zip<W: Write + std::io::Seek>(writer: &mut zip::ZipWriter<W>, base: &Path, current: &Path, options: zip::write::SimpleFileOptions) -> Result<()> {
    ...
    for entry in entries {
        let path = entry.path();
        let rel = path.strip_prefix(base).context("Strip prefix")?.to_string_lossy().replace('\\', "/");

        if path.is_dir() {
            add_dir_to_zip(writer, base, &path, options)?;
        } else {
            writer.start_file(&rel, options).with_context(|| format!("Start ZIP entry: {rel}"))?;
            let mut f = fs::File::open(&path).with_context(|| format!("Open file: {}", path.display()))?;
            let mut buf = Vec::new();
...
pub fn pack_soc_7z(dir: &Path, out_path: &str) -> Result<()> {
    ...
    let mut encoder = sevenz_rust2::ArchiveWriter::create(out_path).with_context(|| format!("Cannot create 7z: {out_path}"))?;
    ...
    for entry in WalkDir::new(dir).into_iter() {
        ...
        let path = entry.path();
        let rel = path.strip_prefix(dir).context("Strip prefix")?.to_string_lossy().replace('\\', "/");

        let src_file = fs::File::open(path).with_context(|| format!("Read file: {}", path.display()))?;
...
pub fn pack_soc(dir: &Path, out_path: &str) -> Result<()> {
    ...
    if info_path.exists() {
        let info: SocInfo = serde_json::from_reader(fs::File::open(&info_path).context("Open info.json")?).context("Parse info.json")?;
...
fn update_script_zip(soc_path: &str, script_data: &[u8], out_path: &str) -> Result<()> {
    ...
    let script_name = {
        let entry = archive.by_name("info.json").context("info.json not found in SOC")?;
        let info: SocInfo = serde_json::from_reader(entry).context("Parse info.json")?;
        info.script.file
    };

    // Rebuild archive with the script entry replaced
    let out_file = fs::File::create(out_path).with_context(|| format!("Create output: {out_path}"))?;
    ...
    for i in 0..archive.len() {
        ...
        writer.start_file(&name, options).with_context(|| format!("Start entry: {name}"))?;
...
fn update_script_7z(soc_path: &str, script_data: &[u8], out_path: &str) -> Result<()> {
    ...
    // Read info.json to find script filename
    let info: SocInfo = serde_json::from_reader(fs::File::open(tempdir.path().join("info.json")).context("info.json missing")?).context("Parse info.json")?;

    // Replace script file
    let script_path = tempdir.path().join(&info.script.file);
    fs::write(&script_path, script_data).with_context(|| format!("Write script: {}", script_path.display()))?;
