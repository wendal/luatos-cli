...
impl SF32LB55Tool {
    ...
    fn download_stub_impl(&mut self) -> Result<()> {
        ...
        let sig_pub_data = ram_stub::find_embedded_stub(SIG_PUB_FILE).ok_or_else(|| {
            tracing::error!("Signature public key file not found: {}", SIG_PUB_FILE);
            std::io::Error::new(std::io::ErrorKind::NotFound, "58X_sig_pub.der file not found")
        })?;

        spinner.set_operation(ProgressOperation::DownloadStub { stage: StubStage::SignatureKey });
        self.download_boot_patch_sigkey(&sig_pub_data)?;
        ...
        spinner.set_operation(ProgressOperation::DownloadStub { stage: StubStage::RamStub });
    ...
    fn wait_for_ok_response(&mut self, timeout_ms: u64) -> Result<()> {
        ...
        loop {
            ...
            if elapsed > timeout_ms {
                ...
                return Err(std::io::Error::new(std::io::ErrorKind::TimedOut, format!("Timeout waiting for OK response: {}", response_str)).into());
            }

            // 每秒记录一次等待状态
            if elapsed > 0 && elapsed.is_multiple_of(1000) && start_time.elapsed().unwrap() > last_log_time.elapsed().unwrap() + Duration::from_secs(1) {
            ...
            if self.port.read_exact(&mut byte).is_ok() {
                ...
                if buffer.windows(4).any(|window| window == b"Fail") {
                    ...
                    return Err(std::io::Error::other(format!("Received Fail response: {}", response_str)).into());
                }
