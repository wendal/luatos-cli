...
新增独立图形界面工具 `luatos-mfgui`，基于 egui 纯 Rust 实现，单可执行文件，适合生产线量产刷机使用。

- **窗口标题** 含版本号：`合宙简易量产刷机程序 v{版本}`
- **固件选取** — 浏览对话框选取 `.soc` 固件文件，自动解析芯片类型和版本号
- **串口选取** — 下拉框 + 刷新按钮；EC718/Air780 系列自动通过 USB VID 检测，无需手动选串口
- **开始/停止** — 开始刷机与中途取消，状态机保证操作互斥
