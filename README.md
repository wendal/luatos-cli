...
- **多芯片刷机** — 支持 Air8101 (BK7258)、Air6208 (XT804)、Air1601 (CCM4211)、Air8000 (EC718) 等模组
...
`luatos-mcp` 是基于 `luatos-cli --format jsonl` 的薄封装 stdio MCP server，适合给 Copilot / Claude Desktop / Inspector 等客户端接入。
...
- `serial_list`
- `soc_info` / `soc_files` / `soc_unpack` / `soc_pack`
- `project_info` / `project_deps` / `project_analyze`
- `build_luac` / `build_filesystem`
- `resource_list` / `resource_download`
- `flash_run` / `flash_script` / `flash_test`
